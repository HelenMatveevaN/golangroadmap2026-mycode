...
func (l *LoggingTaskStore) Save(tasks []Task) error {
...
func (l *LoggingTaskStore) Load() ([]Task, error) {
...
func main() {
	...
	// Базовое хранилище
	baseStore := NewJSONStore("todo.json")

	// Оборачиваем его в декоратор. Весь остальной код main останется прежним!
	var store TaskStore = &LoggingTaskStore{
		TaskStore: baseStore,
	}
